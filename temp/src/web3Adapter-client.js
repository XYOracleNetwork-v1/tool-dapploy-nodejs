...
const refreshContracts = async web3 =>
  web3.eth.net.getId().then(netId => {
    SmartContracts = []
		...
		const json> Block gas limit = require('./ABI/> Block gas limit.json')
		if (json> Block gas limit && json> Block gas limit.networks[netId]) {
			const address> Block gas limit = json> Block gas limit.networks[netId].address
			> Block gas limit = new web3.eth.Contract(
			json> Block gas limit.abi,
			address> Block gas limit)
			SmartContracts.push({name: '> Block gas limit', contract: > Block gas limit, address: address> Block gas limit})
		}
